...
from lib.lake_config import load_lake_config, list_lakes_from_db, LakeConfig
...
def process_lake(config: LakeConfig, wind_speed: float = None, wind_dir: float = None,
                 skip_existing: bool = False, steps: list = None):
    """
    ...
    Args:
        ...
        skip_existing: Skip steps whose outputs already exist
    ...
    # Pass the actual lake name so script 01 can find it in the DB
    if 1 in steps:
        if skip_existing and paths.polygon.exists() and paths.raster.exists():
            logger.info(f"[{lake_id}] Skipping prepare (outputs exist)")
        else:
            ok = run_step([
    ...
    if 2 in steps:
        ...
        if skip_existing and fetch_index.exists():
            logger.info(f"[{lake_id}] Skipping fetch (outputs exist)")
        else:
            ok = run_step([
...
def get_lake_configs(args) -> list:
    """Query the database for lakes matching CLI filters. Returns list of LakeConfig."""
    ...
    # Query DB for multiple lakes
    logger.info(f"Querying database (min area: {min_area} km², "
...
def main():
    ...
    if args.workers <= 1:
        for cfg in configs:
            result = process_lake(
                cfg, args.wind_speed, args.wind_dir,
                args.skip_existing, steps
            )
        ...
        with ProcessPoolExecutor(max_workers=args.workers) as executor:
            futures = {
                executor.submit(
                    process_lake, cfg, args.wind_speed, args.wind_dir,
                    args.skip_existing, steps
                ): cfg.lake_id
                for cfg in configs
            }
